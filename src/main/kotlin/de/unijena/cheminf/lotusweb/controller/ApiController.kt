...
class ApiController(val lotusUniqueNaturalProductRepository: LotusUniqueNaturalProductRepository) {
    ...
    @Autowired
    lateinit var atomContainerToUniqueNaturalProductService:
        AtomContainerToUniqueNaturalProductService

    @Autowired lateinit var pubFingerprintsCountsRepository: PubFingerprintsCountsRepository

    /** Advanced structure handling */
    @RequestMapping("/search/advanced")
    @ResponseBody
    fun advancedSearch(
        @RequestParam("max-hits", required = false) maxHits: String,
        @RequestBody advancedSearchModel: AdvancedSearchModel
    ): Map<String, Any> {
    ...
    fun structureSearchBySmiles(
        @RequestParam("smiles") smiles: String,
        @RequestParam("type") type: String,
        @RequestParam("intaxonomy", required = false) intaxonomy: String?
    ): Map<String, Any> {
        ...
        try {
            if (intaxonomyparam.isNullOrBlank()) {
                ...
                if (type == "smi") {
                    return this.doExactStructureSearchBySmiles(
                        URLDecoder.decode(smiles.trim(), "UTF-8")
                    )
                } else {
                    return this.doExactStructureSearchByInchi(
                        URLDecoder.decode(smiles.trim(), "UTF-8")
                    )
                }
            } else {
                ...
                if (type == "smi") {
                    return this.doExactStructureSearchBySmilesAndTaxonomy(
                        URLDecoder.decode(smiles.trim(), "UTF-8"),
                        intaxonomyparam
                    )
                } else {
                    return this.doExactStructureSearchByInchiAndTaxonomy(
                        URLDecoder.decode(smiles.trim(), "UTF-8"),
                        intaxonomyparam
                    )
                }
    ...
    fun substructureSearch(
        @RequestParam("smiles") smiles: String,
        @RequestParam("type") type: String,
        @RequestParam("max-hits") maxHits: String,
        @RequestParam("intaxonomy", required = false) intaxonomy: String?
    ): Map<String, Any> {
        ...
        try {
            if (intaxonomyparam.isNullOrBlank()) {
                return this.doSubstructureSearch(
                    URLDecoder.decode(smiles.trim(), "UTF-8"),
                    type,
                    maxHits.toIntOrNull()
                )
            } else {
                return this.doSubstructureSearchInTaxa(
                    URLDecoder.decode(smiles.trim(), "UTF-8"),
                    type,
                    maxHits.toIntOrNull(),
                    intaxonomyparam
    ...
    fun similaritySearch(
        @RequestParam("smiles") smiles: String,
        @RequestParam("max-hits") maxHits: String,
        @RequestParam("simThreshold") simThreshold: String,
        @RequestParam("intaxonomy", required = false) intaxonomy: String?
    ): Map<String, Any> {
        ...
        try {
            if (intaxonomyparam.isNullOrBlank()) {
                return this.doSimilaritySearch(
                    URLDecoder.decode(smiles.trim(), "UTF-8"),
                    maxHits.toIntOrNull(),
                    th
                )
            } else {
                return this.doSimilaritySearchInTaxa(
                    URLDecoder.decode(smiles.trim(), "UTF-8"),
                    maxHits.toIntOrNull(),
                    th,
                    intaxonomyparam
    ...
    fun massSearch(
        @RequestParam("minMass") minMass: String,
        @RequestParam("maxMass") maxMass: String,
        @RequestParam("maxHits", required = false) maxHits: String
    ): Map<String, Any> {
        ...
        try {
            ...
            when (ex) {
                ...
                is OutOfMemoryError -> {
                    val other: List<LotusUniqueNaturalProduct> = emptyList()
                    return mapOf("count" to 0, "naturalProducts" to other)
                }
                else -> throw ex
            }
        }
    ...
    */
    ...
    fun doMassSearch(minMass: Double?, maxMass: Double?, maxHits: Int?): Map<String, Any> {
        ...
        if (minMass != null && maxMass != null) {
            ...
            var results =
                this.lotusUniqueNaturalProductRepository.minMaxMolecularWeightSearch(
                    minMass,
                    maxMass,
                    maxResults
                )
            ...
            return mapOf("count" to results.size, "naturalProducts" to results)
        } else if (minMass != null && maxMass == null) {
            println("only min")
            var results =
                this.lotusUniqueNaturalProductRepository.minMolecularWeightSearch(
                    minMass,
                    maxResults
                )
            println(results.size)
            ...
            println("only max")
            var results =
                this.lotusUniqueNaturalProductRepository.maxMolecularWeightSearch(
                    maxMass,
                    maxResults
                )
            println(results.size)
            ...
            return mapOf("count" to 0, "naturalProducts" to results)
        }
    }
    ...
    fun doAdvancedSearch(
        ...
        val results =
            this.lotusUniqueNaturalProductRepository.advancedSearchWithCriteria(
                advancedSearchModel,
                maxResults
            )
    ...
    fun doExactStructureSearchByInchiAndTaxonomy(
        smiles: String,
        intaxonomy: String
    ): Map<String, Any> {
        ...
        try {
            ...
            val results =
                this.lotusUniqueNaturalProductRepository.doExactStructureSearchByInchiAndTaxonomy(
                    queryInchi,
                    intaxonomy
                )
    ...
    fun doExactStructureSearchBySmilesAndTaxonomy(
        smiles: String,
        intaxonomy: String
    ): Map<String, Any> {
        ...
        try {
            ...
            val results =
                this.lotusUniqueNaturalProductRepository.findBySmiles2DOrSmilesAndTaxonomy(
                    smiles,
                    intaxonomy
                )
    ...
    fun doSimpleSearchWithHeuristic(query: String): Map<String, Any> {
        ...
        if (determinedInputType == "SMILES") {
            ...
            try {
                ...
                var queryInchi = gen.getInchi()
                naturalProducts =
                    this.lotusUniqueNaturalProductRepository.findBySmiles2DOrSmiles(query)
                println("original query: " + query)
                println("modified query: " + queryInchi)
                if (naturalProducts.isEmpty()) {
                    println("second try by converting SMILES to InChI")
                    naturalProducts =
                        this.lotusUniqueNaturalProductRepository.findByInchiOrInchi2D(queryInchi)
                ...
                if (lotusPattern.containsMatchIn(query)) {
                    ...
                    naturalProducts =
                        this.lotusUniqueNaturalProductRepository.findByInchiOrInchi2D(query)
                    determinedInputType = "InChi"
                } else if (inchikeyPattern.containsMatchIn(query)) {
                    naturalProducts =
                        this.lotusUniqueNaturalProductRepository.findByInchikeyOrInchikey2D(query)
                    determinedInputType = "InChi Key"
                } else if (molecularFormulaPattern.containsMatchIn(query)) {
                    naturalProducts =
                        this.lotusUniqueNaturalProductRepository.findByMolecular_formula(query)
                    ...
                    naturalProducts =
                        this.lotusUniqueNaturalProductRepository.fuzzyNameSearch(altQuery)
                    // }
    ...
    fun doSubstructureSearchInTaxa(
        smiles: String,
        type: String,
        maxHitsSubmitted: Int?,
        intaxonomy: String
    ): Map<String, Any> {
        ...
        try {
            ...
            val matchedList =
                this.lotusUniqueNaturalProductRepository.findAllPubchemBitsSetWithTaxonomy(
                    queryAC,
                    intaxonomy
                )
            ...
            if (type == "default") {
                ...
                for (unp in matchedList) { // loop@
                    ...
                    var targetAC: IAtomContainer =
                        this.atomContainerToUniqueNaturalProductService.createAtomContainer(unp)
    ...
    fun doSimilaritySearchInTaxa(
        ...
        try {
            ...
            var qmin =
                (Math.ceil(qLen * threshold))
                    .toInt() // Minimum number of bits in results fingerprints
            var qmax = (qLen / threshold).toInt() // Maximum number of bits in results fingerprints
            var ncommon =
                qLen - qmin + 1 // Number of fingerprint bits in which at least 1 must be in common

            var allBits: MutableList<PubFingerprintsCounts> =
                pubFingerprintsCountsRepository.findAll()
            ...
            val matchedList =
                this.lotusUniqueNaturalProductRepository.similaritySearchInTaxa(
                    requestedBits,
                    queryPF,
                    qmin,
                    qmax,
    ...
    fun doSimilaritySearch(smiles: String, maxHitsSubmitted: Int?, th: Int?): Map<String, Any> {
        ...
        try {
            ...
            val matchedList =
                this.lotusUniqueNaturalProductRepository.similaritySearch(
                    requestedBits,
                    queryPF,
                    qmin,
                    qmax,
