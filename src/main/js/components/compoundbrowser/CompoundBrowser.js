...
import CompoundBrowserFilter from "./CompoundBrowserFilter";
import Row from "react-bootstrap/Row";
import Nav from "react-bootstrap/Nav";
import TableCompoundBrowser from "./TableCompoundBrowser";
import CardCompoundBrowser from "./CardCompoundBrowser";
...
export default class CompoundBrowser extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            currentPage: 1
    ...
    withSubscription(ViewComponent, pageNumber, pageRequestHandler, firstPageHandler, prevPageHandler, nextPageHandler, lastPageHandler) {
        return class extends React.Component {
            ...
            fetchNaturalProducts(link = "/api/compound?sort=inchikey,desc&page=" + pageNumber) { // TODO here, see if sort by anything particular
                restClient({
                    method: "GET",
                    path: link
                }).then(
                    (response) => {
                        this.setState({
                            data: {
                                isLoaded: true,
                                result: response.entity._embedded.lotusUniqueNaturalProducts
                            },
                            stats: {
                ...
                if (error) {
                    ...
                    return (
                        <Container>
                            ...
                            <br/>
                            <Row>
                                <p>There are {this.state.stats.totalCompoundCount.toLocaleString()} natural products in the database</p>
                            </Row>
    ...
    render() {
        const CardBrowserWithSubscription = this.withSubscription(CardCompoundBrowser, this.state.currentPage, this.handlePageRequestEvent, this.handlePageFirst, this.handlePagePrev, this.handlePageNext, this.handlePageLast);
        const TableBrowserWithSubscription = this.withSubscription(TableCompoundBrowser, this.state.currentPage, this.handlePageRequestEvent, this.handlePageFirst, this.handlePagePrev, this.handlePageNext, this.handlePageLast);
